...
def llm_rank_zones(
    ...
    """
    Ask the LLM to rank the top_n ground-truth-scored zones.
    Returns {"llm_ordered_ids": list[str], "error": str | None}
    """
    candidates = ranked_zones[:top_n]
    if not candidates:
        return {"llm_ordered_ids": [], "error": "No candidates"}
    ...
    try:
        ...
        # Map short labels back to real zone IDs
